...
public abstract class AbstractServerClient implements FileClient {
    ...
    public String getAccessPath(String objectName) {
        ...
        if (StringUtils.lastIndexOf("/", webServerUrl) == -1) {
            return webServerUrl + "/" + objectName;
        }
        return webServerUrl + objectName;
    }
    ...
    public String uploadFile(String url, String yourObjectName) {
        try (InputStream is = new URL(url).openStream()) {
            return this.uploadFile(is, yourObjectName);
        } catch (IOException e) {
            throw new RuntimeException("upload url file error", e);
        }
    }

    @Override
    public void copyFile(File inputFile, File outFile) throws IOException {
    ...
    @Override
    public void copyFile(InputStream inputStream, File outFile) throws IOException {
    ...
    @Override
    public void chunkFile(File file, String chunkPath, long chunkSize) throws IOException {
        // 分割文件的数量
    ...
    @Override
    public String mergeFile(String chunkPath, File mergeFile) throws IOException {
        //合并文件
        ...
        //分块列表
        List<File> mergeFileList = this.getMergeFileList(chunkPath, Comparator.comparing(o -> Integer.parseInt(o.getName())));
        //合并文件
    ...
    private List<File> getFileList(String chunkPath) {
        //块文件目录
    ...
    @Override
    public List<File> getMergeFileListByNumber(String chunkPath) {
        List<File> files = this.getFileList(chunkPath);
        files.sort(Comparator.comparingInt(o -> Integer.parseInt(o.getName())));
        return files;
    }

    @Override
    public List<File> getMergeFileListByName(String chunkPath) {
        List<File> files = this.getFileList(chunkPath);
    ...
    @Override
    public List<File> getMergeFileList(String chunkPath, Comparator<File> comparator) {
        List<File> files = this.getFileList(chunkPath);
    ...
    @Override
    public List<File> getMergeFileList(String chunkPath, FileFilter fileFilter, Comparator<File> comparator) {
        //块文件目录
        File chunkFolder = new File(chunkPath);
        //分块列表
